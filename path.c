...
static void process_neibor(V3i p1, V3i p2, int clearence)
{
  ...
  {
    Dir d = m2dir(p1, p2);
    if (d == D_F  && (check_wall(p1, 0) || check_wall(p2, 2))) {
      return;
    }
    if (d == D_R  && (check_wall(p1, 1) || check_wall(p2, 3))) {
      return;
    }
    if (d == D_B  && (check_wall(p1, 2) || check_wall(p2, 0))) {
      return;
    }
    if (d == D_L  && (check_wall(p1, 3) || check_wall(p2, 1))) {
      return;
    }
    if (d == D_FR && (check_wall(p1, 0) || check_wall(p1, 1)
        || check_wall(p2, 2) || check_wall(p2, 3))) {
      return;
    }
    if (d == D_BR && (check_wall(p1, 1) || check_wall(p1, 2)
        || check_wall(p2, 0) || check_wall(p2, 3))) {
      return;
    }
    if (d == D_BL && (check_wall(p1, 2) || check_wall(p1, 3)
        || check_wall(p2, 0) || check_wall(p2, 1))) {
      return;
    }
    if (d == D_FL && (check_wall(p1, 3) || check_wall(p1, 0)
        || check_wall(p2, 2) || check_wall(p2, 1))) {
      return;
    }
  }
